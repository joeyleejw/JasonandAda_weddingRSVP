const translations = {
   en: {
      rsvpMain: "J&A",
      // welcome section
      welcomeHeadline: "WE ARE GETTING MARRIED!",
      welcomeSubheadline: "We would be honoured to surround ourselves with the people we love in celebration of our joyous union.",
      lovelyCouple: "JASON & ADA",
	  rsvpButton: "RSVP",

      // rsvp section
      RSVPbefore: "by 30/10/2026",
      RSVPheadline: "Our Big Day is Almost HERE!",
      RSVPsubheadline: "With hearts full of joy, we invite you to witness our forever in",
      countdownDays: "Days",
      countdownHours: "Hours",
      countdownMinutes: "Minutes",
      countdownSeconds: "Seconds",
      RSVPdate: "12/12/2026 (Saturday)",
      RSVPreception: "Reception — 6:00pm",
      RSVPdinner: "Dinner — 7:00pm",
      RSVPlocation: "Xin Cuisine Chinese Restaurant, Concorde Hotel",

      //rsvp form
      formHeadline: "Save Your Seat at Our Table",
      formSubheadline: "Kindly RSVP",
      formFullname: "Full name",
      formInvitedBy: "Invited by",
      formWhichSide: "Select",
      formBride: "Bride",
      formGroom: "Groom",
      formAttend: "Will you be attending?",
      formAttendselect: "Select",
      formAttendyes: "Yes",
      formAttendno: "No",
      formGuestTitle: "Guest {n}",
      formGuestsNumber: "Amount of guests (including yourself)",
      formPhoneNumber: "Phone Number",
      formPhoneError: "Please enter a complete phone number.",
      formMealLabel: "Meal Preference",
      formMealSelect: "Select",
      formMealNonHalal: "Non-Halal",
      formMealHalal: "Halal",
      formMealVegetarian: "Vegetarian",
      formGuestsName: "Full Name",
      formHasChildren: "Bringing children below age of 12?",
      formGuestsChildren: "How many children?",
      formBabyChairNeed: "Baby chair required?",
      formBabyChairNo: "No",
      formBabyChairYes: "Yes",
      formBabyChairYesNum: "How many baby chair(s)?",
      formSendUsBlessingTitle: "Sending Us Your Blessings",
      formSendUsBlessingSubtitle: "We understand you can't join us, but we'd love to hear from you!",
      placeholderMessage: "Wishing you a lifetime of love and happiness...",
      buttonSubmit: "Submit RSVP",
	  buttonSubmitting: "Submitting...",

      // footer
      authorCopyright: "Author: Joey Lee @ 2026",
      contactUstext: "Contact Us",
      contactUsmale: "Jason",
      contactUsfemale: "Ada",

      // modal
      modalTitle: "Wedding RSVP",
      modalParkingTitle: "Parking Information",
      modalParkingStep1: "From the lobby, turn left into the parking entrance — pay with TnG or a debit card.",
      modalParkingStep2: "Once parked, head to Level 2B and follow the signs for \u201cAccess to Link Bridge.\u201d",
      modalParkingStep3: "Cross the bridge and follow the signage down to the reception — we'll be there to greet you.",
      modalParkingNote: "Parking: RM12.00 nett flat rate per car, per entry.",
      parkingSummary: "Level 2B, accessible via the Lobby and Link Bridge",
      modalDownload: "Download",
      modalShare: "Share",
      modalAddCalendar: "Add to Calendar",

      // confirmation modal/card — dynamic summary content
      thankingRSVP: "thank you for your RSVP!",
      summaryLabelAttending: "Attending",
      summaryLabelGuests: "Guests",
      summaryLabelTotalGuests: "Total Guests",
      summaryLabelGuestNames: "Guest Names",
      summaryLabelChildren: "Children",
      summaryLabelChildrenBelow12: "Children Below 12",
      summaryLabelBabyChairs: "Baby Chairs",
      summaryLabelBabyChairNeeded: "Baby Chair Needed",
      summaryLabelMeals: "Meals",
      summaryLabelMessage: "Message",
      mealNone: "None",
      summaryPaxValue: "{n} pax",
      summaryYesPaxValue: "Yes, {n} pax",
      summaryYesValue: "Yes, {n}",

      //confirmation card
      confirmationConfirm: "RSVP CONFIRMED",
      confirmationDateTime: "12/12/2026 (Saturday), 6PM",
   },

   zh: {
      rsvpMain: "J&A",
      // welcome section
      welcomeHeadline: "我們要結婚了！",
      welcomeSubheadline: "我們誠摯邀請您與我們最親愛的人們相聚，共同慶祝這份喜悅與結合。",
      lovelyCouple: "傑晟 與 張融",
	  rsvpButton: "敬請回覆",

      // rsvp section
      RSVPbefore: "請於 2026年10月30日前回覆",
      RSVPheadline: "我們的大喜之日即將到來！",
      RSVPsubheadline: "懷著滿心的喜悅，誠邀您見證我們的永恆於",
      countdownDays: "天",
      countdownHours: "小時",
      countdownMinutes: "分鐘",
      countdownSeconds: "秒",
      RSVPdate: "2026年12月12日（星期六）",
      RSVPreception: "迎賓 — 晚上 6:00",
      RSVPdinner: "晚宴 — 晚上 7:00",
      RSVPlocation: "新故鄉酒樓，吉隆坡協和酒店",

      // rsvp form
      formHeadline: "為您留一席之位",
      formSubheadline: "敬請回覆",
      formFullname: "姓名",
      formInvitedBy: "受邀方",
      formWhichSide: "請選擇",
      formBride: "新娘",
      formGroom: "新郎",
      formAttend: "您是否出席？",
      formAttendselect: "請選擇",
      formAttendyes: "是",
      formAttendno: "否",
      formGuestTitle: "賓客 {n}",
      formGuestsNumber: "出席人數（包含您本人）",
      formPhoneNumber: "電話號碼",
      formPhoneError: "請輸入完整的電話號碼。",
      formMealLabel: "餐點偏好",
      formMealSelect: "請選擇",
      formMealNonHalal: "非清真",
      formMealHalal: "清真",
      formMealVegetarian: "素食",
      formGuestsName: "姓名",
      formHasChildren: "是否攜帶 12 歲以下兒童？",
      formGuestsChildren: "兒童人數？",
      formBabyChairNeed: "是否需要嬰兒椅？",
      formBabyChairNo: "否",
      formBabyChairYes: "是",
      formBabyChairYesNum: "需要幾張嬰兒椅？",
      formSendUsBlessingTitle: "為我們送上祝福",
      formSendUsBlessingSubtitle: "我們理解您無法出席，但仍非常希望收到您的祝福！",
      placeholderMessage: "祝福你們永浴愛河，幸福美滿……",
      buttonSubmit: "提交回覆",
	  buttonSubmitting: "提交中……",

      // footer
      authorCopyright: "作者：Joey Lee @ 2026",
      contactUstext: "聯繫我們",
      contactUsmale: "Jason",
      contactUsfemale: "Ada",

      // modal
      modalTitle: "婚禮回覆",
      modalParkingTitle: "停車資訊",
      modalParkingStep1: "從大堂出發，向左轉進入停車場入口 — 可使用 TnG 或簽帳卡（Debit Card）付款。",
      modalParkingStep2: "停好車後，請前往 2B 樓層，並留意「通往連接橋」的指示牌。",
      modalParkingStep3: "穿過連接橋，沿指示牌走向宴會廳 — 我們將在那裡迎接您。",
      modalParkingNote: "停車費：每輛車每次進場收費 RM12.00（淨價，統一收費）。",
      parkingSummary: "2B 樓層，經大堂與連接橋可達",
      modalDownload: "下載",
      modalShare: "分享",
      modalAddCalendar: "新增至日曆",

      // confirmation modal/card — dynamic summary content
      thankingRSVP: "感謝您的回覆！",
      summaryLabelAttending: "是否出席",
      summaryLabelGuests: "出席人數",
      summaryLabelTotalGuests: "出席總人數",
      summaryLabelGuestNames: "賓客姓名",
      summaryLabelChildren: "兒童人數",
      summaryLabelChildrenBelow12: "12歲以下兒童",
      summaryLabelBabyChairs: "嬰兒椅數量",
      summaryLabelBabyChairNeeded: "是否需要嬰兒椅",
      summaryLabelMeals: "餐點",
      summaryLabelMessage: "留言",
      mealNone: "無",
      summaryPaxValue: "{n} 位",
      summaryYesPaxValue: "是，{n} 位",
      summaryYesValue: "是，{n}",

      // confirmation card
      confirmationConfirm: "已確認回覆",
      confirmationDateTime: "2026年12月12日（星期六），晚上 6點",
   },
};
